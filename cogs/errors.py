...
class Errors(commands.Cog):
    ...
    async def on_command_error(self, ctx:commands.Context, error:Exception):
        if isinstance(error, commands.CommandNotFound):
            ...
            await ctx.reply(
                embed=discord.Embed(title=f"{ctx.command.full_parent_name}{ctx.invoked_with} is missing an argument", 
                color=0xff0000,
                description=f"**Usage:**\n{ctx.prefix}{ctx.command.full_parent_name}{ctx.invoked_with} {ctx.command.signature}"
                ).set_footer(
                    text=f"Type {ctx.prefix}help {ctx.command.full_parent_name}{ctx.invoked_with} for more info"
                )
            )
        elif isinstance(error, commands.BadArgument):
            await ctx.reply(
                embed=discord.Embed(title=f"{ctx.command.full_parent_name}{ctx.invoked_with} is using an invalid argument", 
                color=0xff0000,
                description=f"**Usage:**\n{ctx.prefix}{ctx.command.full_parent_name}{ctx.invoked_with} {ctx.command.signature}"
                ).set_footer(
                    text=f"Type {ctx.prefix}help {ctx.command.full_parent_name}{ctx.invoked_with} for more info"
